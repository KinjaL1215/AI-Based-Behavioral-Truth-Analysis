    ...
    try:
        while cap.isOpened():
            ...
            if result.face_landmarks:
                # IMPORTANT: Get the raw MediaPipe landmark objects first
                landmarks_raw = result.face_landmarks[0]
                
                # Convert to (x, y) tuples ONLY for the tension detector
                landmark_points = [(lm.x, lm.y) for lm in landmarks_raw]
                
                # 1. BLINK DETECTION (Use raw objects so .x and .y work)
                ...
                if frame_counter < calibration_frames:
                    ...
                    # Blink Calibration (Natural eye opening)
                    ...
                    if final_baseline_tension is None:
                        ...
                        # Set dynamic threshold based on user's eye shape
